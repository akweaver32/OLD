#ADAPTING SOPHIE'S CODE

#installing packages and setting up libraries
packages_cc <- c("dlnm", "tidyverse", "Epi", "gnm", "ggplot2", "parallel", "sf", "raster")
lapply(packages_cc, library, character.only = TRUE)

##############################################################################################################

#read in case data (clean_cocci_data.R)
#as RDS object

#establish hazard length (another option is 42)
hazard_length <- 60

#create function to get control periods (sophies for now)
getControlPeriods <- function(i) {
  cs <- cases[i,]
  dates <- cs$est_date - (1:4 * 365)
  ndates <- length(dates)
  dates <- dates[dates != cs$est_date]
  return(data.frame("CID" = rep(cs$CID, ndates), 
                    "date" = dates, 
                    "case" = 0))
}

#run creation of control periods parallel
cl <- makeCluster(3, setup_strategy = "sequential")
clusterExport(cl, varlist = c("getControlPeriods", "cases"))
tmp <- parLapply(cl, 1:nrow(cases), getControlPeriods) # 30s to run, tmp = temporary
stopCluster(cl)

#compile look up what do.call means
begin <- Sys.time()
controls <- do.call(rbind, tmp)
end <- Sys.time()
print(end - begin) # 1.5 mins to run (double check this)

#write function for ambidirectional control periods (ideally want to do time stratified)
cp_test <- function(i){
  cs <- cases[i,]
  dates <- c(cs$date - (1:2 * 365), cs$date + (1:2 * 365)) #this line does something a little weird - double check
  ndates <- length(dates)
  dates <- dates[dates != cs$date]
  return(data.frame("CID" = rep(cs$CID, ndates), 
                    "date" = dates, 
                    "case" = 0))
}

#test with your new function (works)
cl <- makeCluster(3, setup_strategy = "sequential")
clusterExport(cl, varlist = c("cp_test", "cases"))
tmp <- parLapply(cl, 1:nrow(cases), cp_test) # 30s to run
stopCluster(cl)

#compile look up what do.call means
begin <- Sys.time()
controls <- do.call(rbind, tmp)
end <- Sys.time()
print(end - begin) # 2.5 min to run

#bind the cases and control periods together
casemat <- rbind(cases %>%
                   dplyr::select(CID, date) %>%
                   mutate(case = 1), 
                 controls)

casemat$CID <- as.numeric(casemat$CID)

case_control <- casemat %>% #with 10k buffers
  arrange(CID) %>%
  left_join(cases_buffer, by = "CID") %>% #buffers in 3488
  rename(cc_date = date.x, org_date = date.y) %>%
  mutate(lagged_cc_date = cc_date - days(60)) #lag dates for hazard period
  
#save as CSV file
write_csv(case_control, "case_control_lagged60.csv")

cc_sand <- left_join(case_control, buffer_sand_10k, by = c("CID" = "GEOID")) %>%  #with 10k buffers and 10k vals
  rename(per_sand = mean) %>%
  select(-10)

cc_sand$test <- runif(272070) #to test clogistic, works

clOut <- Epi::clogistic(case ~ test, strata = CID, data = cc_sand)

cc_2000 <- Epi::clogistic(case ~ mean_wind_speed, strata = CID, data = test)
